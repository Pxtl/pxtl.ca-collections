namespace PxtlCa.Collections.DictMixins {
    public class ChangeEventDictMixin<K, V> : DictMixin<K, V> {
        ...
        public override void SetVal(IDictionary<K, V> thisDict, IDictMixinNode<K, V> nextMixin, K key, V value) {
            nextMixin[key] = value;
            NoteChange(key, ChangeType.Set, value);
        }

        // Delegating implementations of all other methods.
        public override bool Remove(IDictionary<K, V> thisDict, IDictMixinNode<K, V> nextMixin, K key) {
            NoteChange(key, ChangeType.Remove, default);
            return nextMixin.Remove(key);
        }
        public override void Add(IDictionary<K, V> thisDict, IDictMixinNode<K, V> nextMixin, K key, V value) {
            nextMixin.Add(key, value);
            NoteChange(key, ChangeType.Add, value);
        }
        public override bool Remove(IDictionary<K, V> thisDict, IDictMixinNode<K, V> nextMixin, KeyValuePair<K, V> item) {
            NoteChange(item.Key, ChangeType.Remove, default(V));
            return nextMixin.Remove(item);
        }

        public override void Clear(IDictionary<K, V> thisDict, IDictMixinNode<K, V> nextMixin) {
